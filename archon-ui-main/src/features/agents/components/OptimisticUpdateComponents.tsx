...
export const OptimisticStatus: React.FC<OptimisticStatusProps> = ({
  ...
  switch (status) {
    ...
    case 'success':
      return (
        <CheckCircle
          className={cn(iconClass, 'text-green-500', className)}
        />
      );
    case 'error':
      return (
        <XCircle
          className={cn(iconClass, 'text-red-500', className)}
        />
      );
    default:
      return (
        <AlertCircle
          className={cn(iconClass, 'text-gray-400', className)}
        />
      );
  }
};
...
export const OptimisticButton: React.FC<OptimisticButtonProps> = ({
  ...
  const getIcon = () => {
    if (isLoading || isOptimistic) return <Loader2 className="w-4 h-4 animate-spin mr-2" />;
  ...
  return (
    <button
      className={cn(
        ...
        {
          'bg-blue-600 text-white hover:bg-blue-700': !isLoading && !isOptimistic && !hasError,
          'bg-blue-400 text-white cursor-not-allowed': isLoading || isOptimistic,
    ...
    <div
      className={cn(
        'transition-all duration-300 ease-in-out',
        {
          ...
          'opacity-100 scale-100 transform translate-y-0': !isOptimistic && !isRemoving,
        },
        className
      )}
    >
  ...
  return (
    ...
    >
      <div className="flex items-center">
        <OptimisticStatus status={isOptimistic ? 'optimistic' : type === 'success' ? 'success' : 'error'} />
