...
async def get_providers_list(
    ...
    try:
        ...
        if not all_providers:
            raise HTTPException(status_code=404, detail="No providers available from OpenRouter API")
            
        logger.info(f"Retrieved {len(all_providers)} providers from OpenRouter and local sources")
        ...
        try:
            async with sync_service.uow as uow:
                ...
                if providers:
                    logger.info(f"Fallback: Retrieved {len(providers)} providers from database")
                    return providers
                else:
                    raise HTTPException(status_code=404, detail="No providers found in database or OpenRouter")
        except Exception as db_error:
            logger.error(f"Database fallback also failed: {db_error}")
            raise HTTPException(status_code=500, detail=f"Failed to get providers from both OpenRouter and database: {str(e)}")
