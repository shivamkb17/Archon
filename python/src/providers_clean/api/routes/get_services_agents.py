    ...
    try:
        ...
        try:
            if await sync_service.should_sync(max_age_hours=24):
                asyncio.create_task(sync_service.full_sync(force_refresh=False))
                asyncio.create_task(registry_service.sync_registry_with_model_configs())
        except Exception:
            pass
        agents = await registry_service.get_agents(active_only=active_only)
        if not agents:
            raise HTTPException(status_code=404, detail="No agents found in database")
        return agents
